...
새 일정을 등록합니다. 시스템이 자동으로 4자리 숫자 출첵 코드(`checkCode`)를 생성합니다. 회원 인증이 필요하며, 일반 회원(`USER`)은 `category=GROUP_ACTIVITY`인 일정만 생성할 수 있습니다. 그 외 카테고리는 운영진(`MANAGER`)만 생성할 수 있습니다.
...
| `checkCode` | 문자열 | (선택) 출첵 코드 수동 지정. 미전송 시 4자리 숫자로 자동 생성                                              |
...
| `checkCode` | 문자열 | 자동 생성된 4자리 숫자 출첵 코드 |
| `author`    |  숫자  | 작성자 회원 ID                   |
...
2. `checkCode` 미전송 시 4자리 숫자로 자동 생성된다
